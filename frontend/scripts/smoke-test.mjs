const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:4530';
...
const adminCookie = await login('admin@wealth-advisory.local', 'admin123');
const managerCookie = await login('manager@wealth-advisory.local', 'manager123');
const analystCookie = await login('analyst@wealth-advisory.local', 'analyst123');

await expectJson('/api/dashboard', adminCookie);
await expectJson('/api/entities/portfolios', analystCookie);
await expectJson('/api/documents', analystCookie);
await expectStatus('/api/documents/upload', analystCookie, 405);

const records = await expectJson('/api/entities/portfolios', managerCookie);
const rowId = records.rows[0].id;

const approveResponse = await fetch(`${baseUrl}/api/entities/portfolios/approve`, {
  method: 'POST',
  headers: {
...
const forbiddenApprove = await fetch(`${baseUrl}/api/entities/portfolios/approve`, {
  method: 'POST',
  headers: {
